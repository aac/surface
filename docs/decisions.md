...
## 2026-05-29 · Detached reaction for long-gap surfaces; hold-open stays correct for short windows

**Request:** Andrew asked whether a background session can mint a surface that a recipient interacts with hours later, after the session has timed out — and whether surface should, in some situations, create a lightweight script/binary for polling/pushing that only invokes an agent when there's something to react to.

**Decision:** Named the **mint-session ≠ react-session** split explicitly in `lifecycle.md`. Established the **detached regime** (mint, persist, detach, arm trigger, exit; react later in a fresh agent) as the correct model for any gap longer than a held-open session — distinct from **hold-open** (Monitor / tight ScheduleWakeup in the live session), which stays correct for short windows. Recorded two sharpened requirements for the detached regime (the intent map must carry reaction context for a context-free agent; reaction results need an outbound channel since the minting chat is gone) and one hazard (state decays between mint and reaction — the reactor re-validates against the live world). Documented the **non-agent detector that gates a fresh agent** (Andrew's lightweight script/binary) as an **optional optimization**, not a new invariant — *encouraged* via a recognition cue (named triggers: long gap, sparse submissions, async multi-round; the anti-pattern it removes is a detached drain that wakes a full agent per poll tick), never prescribed. How hard to lean on that cue is left to be tuned from the clean-room test's measured discovery rate (act-eb9f02) rather than decided up front. Minor bump 0.4.2 → 0.5.0 across SKILL.md + both manifests.

**Reasoning:** The pattern already supported detached draining (fs-drop queues a backlog; hosted poll has no held-open agent), but the docs never named the agent-identity split or the token economics of starting a fresh agent vs. resuming a fat session. The harness's own "don't resume long-idle sessions" warning makes this load-bearing: the answer to "keep the session alive for a day" is "don't — make reaction stateless." The detector idea is genuinely useful but had to be framed as an optimization, not a prescription: mandating a sentinel binary, or spelling out its spawn command / auth / return-path, would violate the trust-the-agent and non-prescriptive core principles — those stay implementation-defined. On *encouraging* the detector: agents are unlikely to discover it spontaneously because it is a pure cost optimization with no in-loop feedback gradient — an agent optimizes for "did I react?" not "did I react cheaply?", and the wasteful alternatives (hold-open, wake-an-agent-per-tick) pass every short test, so the waste is invisible exactly in the long-running and multi-round cases where it compounds. Surfacing the choice and its trigger is squarely the skill's job ("surfaces choices; lists of options, not recipes") and is distinct from prescribing the mechanism — what agents under-discover is the *recognition* that per-tick agent wakes are wasteful, not the *construction* of a watcher. **Rejected alternative:** making hold-open ("stay alive and drain") the canonical lifecycle model — rejected because it breaks on multi-hour gaps, burns tokens idling, and collides with the resume warning. Hold-open is kept as the correct short-window regime, not the default for all surfaces.
