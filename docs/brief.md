...
**The substrate-agnostic test (methodology).** The validating question for any alternative-substrate port is "can the agent build a working poke-like server from `SKILL.md`, `references/`, and the brief alone?" — not "does it match an existing sibling impl byte-for-byte?" Dispatching a fresh references-only port with instructions to "mirror Go" (or "mirror Python," or "match the existing impl") defeats the test at the dispatch layer: the agent ends up cloning a sibling's operational choices rather than independently deriving them. The correct framing is references-only with no sibling impls in context; operational divergence on port, watchdog, error statuses, body-cap policy, Cache-Control specifics is signal of validation, not failure. Convergence on the wire envelope (state schema, SUBMIT line shape, multipart field name, RFC3339 timestamps) is signal the docs pinned the right things. Cross-impl byte-equivalence is informational only — useful for finding doc ambiguity, not a pass/fail bar.

The act-a063 closure (Rust references-only port surfaced 5 doc gaps → spawned act-b488 / b48e / 4f3e / 3ef7 / c281) was a mixed bag. Some of those gaps were genuine doc ambiguity worth pinning; others (HTTP 415 for unsupported content type, 413 status for body-cap rejection) were over-prescription that got pinned in the doc-coherence pass and later relaxed back to implementation-defined in act-087a. The lesson: when a references-only port surfaces a "gap," ask whether the gap is doc ambiguity (pin it) or operational variation the substrate is correctly absorbing (call it out as implementation-defined, keep convention-of-existing-impls as informational). Default to implementation-defined; pin only what genuinely belongs on the wire.
...
- **Cross-implementation interop testing.** No spec, no conformance suite, no tests beyond "the skill works for real tasks." Cross-impl smoke checks (do all sibling impls accept the same submission JSON, emit the same SUBMIT line shape?) are informational only — see the "substrate-agnostic test (methodology)" note above; convergence on the wire envelope is useful evidence, divergence on operational details is not a failure.
