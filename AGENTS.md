...
**Don't hand-edit the four strings** — run the kit's `bin/bump-version <new-version> --repo .`, which rewrites all four in place, then add the `CHANGELOG.md` entry it reminds you about. The kit's `bin/check-versions --repo .` verifies the four are in lockstep and is the single source of truth for the check. CI enforces both halves: it checks out `plugin-release-kit` pinned to an **immutable commit SHA** (never a moving branch/tag — same supply-chain hygiene the kit's `verify-release [7]` enforces) and calls `bin/check-versions`, plus a `CHANGELOG.md` entry must exist for the current version — so the changelog can't silently rot behind the manifests, as it did up to 0.6.1. `.githooks/pre-commit` runs the same `bin/check-versions` locally before push when a kit checkout is present (at `$PLUGIN_RELEASE_KIT` or a sibling `../plugin-release-kit`), and degrades gracefully to CI-as-gate when it isn't. (Before this was wired up, `marketplace.json`'s `version` was the one string CI *didn't* check, and it silently drifted to 0.8.0 while the other three sat at 0.8.1.)
