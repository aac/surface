...
`skills/surface/SKILL.md` frontmatter carries a `version:` field. **Both** plugin manifests — `.claude-plugin/plugin.json` and `.codex-plugin/plugin.json` — carry the same `version`. The shared version string lets anyone eyeball-compare what's loaded in a harness (Cowork, Claude Desktop, Codex) against what's in this repo without diffing the full bytes.

**Rule:** any landing that changes skill content (`skills/surface/SKILL.md`, anything under `skills/surface/references/`, anything under `skills/surface/examples/`) bumps **all three** `version:` strings together (SKILL.md + both plugin manifests) **and adds a matching `## [<version>]` section to `CHANGELOG.md`**. Patch-bump for content tweaks, minor-bump for new rules / new references / new examples / shape changes. Keep them lockstep — if they ever drift, the comparison signal dies. CI enforces both halves: the three version strings must match, and `CHANGELOG.md` must carry an entry for the current version (so the changelog can't silently rot behind the manifests, as it did up to 0.6.1).
