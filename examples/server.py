...
# JSON above this returns 413 too. Ephemeral poke surfaces rarely need more.
MAX_BODY_BYTES = 32 * 1024 * 1024  # 32 MiB

# Where to store uploaded files. The wire contract requires "any absolute path
# the agent can read back"; the references suggest a shared ``poke-uploads``
# directory under tempdir, which is what we use.
UPLOAD_DIR = Path(tempfile.gettempdir()) / "poke-uploads"
...
@dataclass
class ServerConfig:
    """Runtime configuration for one poke session.
...
def _atomic_write_json(path: Path, payload: Any) -> None:
    ...
    poke state is ephemeral, and the wire contract names rename-atomicity as
    the requirement.
    """
...
class PokeServer(ThreadingHTTPServer):
    ...
    def __init__(
        ...
        super().__init__((config.bind, config.port), PokeHandler)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a poke reference server for one session.",
    )
    parser.add_argument(
        "--state",
        required=True,
...
def main(argv: list[str] | None = None) -> int:
    ...
    server = PokeServer(config)
    print(
        f"poke server listening on http://{config.bind}:{config.port}/",
        file=sys.stderr,
    )
    try:
