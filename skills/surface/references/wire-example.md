...
| Method | Path | Purpose |
|---|---|---|
| `GET` | `/` | Serves the agent-rendered HTML for the current surface session. |
...
- **`session_id`** — opaque identifier scoping this surface's affordances from any
  other concurrent surface. Generated when the session starts; meaningful only to
...
state to `/tmp/surface-state.json`:
...
```
server --state /tmp/surface-state.json --html /tmp/surface-page.html --port 5173
```
...
teardown, concurrent surfaces, browser caching, state file lifecycle, retry
