...
Autonomous draining is foundational: once a surface is live, the agent must learn about submissions and react on its own. The *mechanism* is the agent's choice. This reference describes the mechanism space and walks through concrete options. Pick what fits the environment.
...
**Codex:** spawn the server under the long-running tool pattern (a background shell the tool keeps alive), then tail its stdout in a loop using the subprocess tail primitive, reading one `SUBMIT` line at a time. The decoupled shape is the same — server lifetime and drain lifetime stay separate. The tail loop must run inside a live turn, though: where the environment doesn't wake the agent on background stdout, submissions buffer unobserved between turns, so ending the turn once the server is up halts the drain even as the server keeps accepting them — hold the turn open or detach (see "Mint lifetime vs. react lifetime").
...
The server and the drain are separate concerns with different lifetimes: the server stays alive as long as the surface is active, while the drain must be re-armable without restarting the server. Don't couple them — launching the server *inside* a Monitor command, so it dies when the Monitor times out, is the fragile shape (the surface goes offline whenever the drain re-arms). Start the server via `run_in_background` (or `nohup`, or `--drain-mode fs`) and point the drain at it separately; the worked Monitor example below shows the decoupled shape.

## Mint lifetime vs. react lifetime

The agent that *mints* a surface and the agent that *reacts* to a submission need not be the same agent — and across any gap longer than a held-open session, they must not be.

**Hold-open.** The minting session stays live and drains in place — Monitor on the server's stdout, or a tight `ScheduleWakeup`/`/loop` cadence. Near-zero latency; the cost is the session idling. Correct for seconds-to-about-an-hour windows with a present recipient. Hold-open means the *turn* stays open, not merely the server: the drain only advances while the session is actively polling inside a live turn. Ending the turn stops the drain even though the background server keeps accepting submissions — *server still up* is not *agent still draining*; anything landing after the turn ends buffers unobserved until something reopens a turn. So hold-open requires Monitor blocking the turn open, or a tight within-session wake cadence. `ScheduleWakeup` wakes *this* session (a within-session timer, not detachment) — across a multi-hour gap that is both a robustness problem and a cost one (re-reading a day of transcript per wake).

**Detached.** Minting and reaction are separate agent invocations. The minting session renders the surface, persists the intent map, detaches the server/store so it outlives the session, arms a trigger, delivers the URL, and **exits** — it does not wait. Reaction happens later in a *fresh agent* that loads the durable state rather than resuming the minting session.

The regime choice is orthogonal to interactive-vs-autonomous invocation. The detached regime sharpens two things hold-open lets you cheat on, and adds one hazard:

- **The intent map must carry reaction context, not just labels.** A fresh agent has no warm memory of why the surface exists. The persisted state must say, per affordance, not only *what was chosen* but *what reacting means* — which repo, which action, where the result goes.
- **Reaction results need an outbound channel.** The minting chat is gone; the reactor delivers through whatever outbound channel it has (`reach` preferred).
- **State decays between mint and reaction.** The world can change while the surface is live. The intent map carries *what to do if conditions still hold*, not *whether they hold*; a fresh agent should re-validate against the live world before acting.

Mechanism-to-regime mapping: Monitor and tight `ScheduleWakeup`/`/loop` are hold-open; a scheduled fresh-agent (cron/routine) poll, or a push-webhook that spawns a fresh agent, are detached. The fs drop-directory is regime-neutral.

**Gating a fresh agent with a non-agent detector (optional).** In the detached regime, the cheapest correct shape is often a *non-agent* process between the surface and the agent — a few lines of shell, a small binary, or the surface's own server watches for a submission and invokes an agent *only when one lands*. This collapses two wastes: the idle-session cost of holding an agent open, and the empty-tick token cost of waking a full agent on every poll to find nothing. Locally, the server (or a watcher over the `--drain-mode fs` drop-directory) spawns the reacting agent on submit; in a hosted setup the Worker already *is* such a detector (push-webhook is its ideal form). An optimization, not an invariant — reach for it when the gap is long, submissions are sparse, or the surface runs multiple rounds spaced in human time. The axis is what each wake-up wakes, not poll-vs-push. One caveat the held-open loop doesn't have: two submissions landing close together can spawn two agents over the same state, so serialize or dedupe if that matters.

**When no agent exists at submit time: fold the reaction into a daemonized server.** Some environments can't guarantee even that an agent can be *spawned* on submit. In that case move the reaction *into the persistent server process itself*: on a valid submission the server writes the result file directly — it *is* the autonomous drain, with no agent in the loop. Two requirements make it survive the minting session's death:

- **Daemonize.** Double-`fork()` + `setsid()` so the process reparents to init (PPID 1) and detaches from the launching session's process group. Verify with `ps -o ppid=` showing `1`.
- **No parent-death watchdog.** A server built for a held-open drain often ships a watchdog that shuts down when its launching parent exits — correct there, fatal here. Omit it.

Useful touches: bind the port *before* daemonizing so a conflict still surfaces to the launching shell; a first-valid-submission-wins guard that re-derives "already decided" from the result file's existence at startup; and a self-shutdown a few seconds after recording so the HTTP response flushes first. The honesty rule (Designing affordances Rule 4) binds hardest here: with no agent to react, confirmation copy says "saved for the operator," never "an agent will pick this up." State kept in a harness-managed scratch/job directory inherits that harness's cleanup lifecycle, so write the durable result somewhere the harness won't reap.
...
shell_id = Bash(
    command          = "go run skills/surface/examples/server.go "
...
spawn_detached(
    "go run skills/surface/examples/server.go "
...
The question isn't "has the timeout elapsed?" — it's "do I have enough signal to conclude this surface isn't going to get submissions?" Useful signals: time since delivery, whether the URL was ever opened, the recipient's likely schedule, and whether the task is still relevant. Each mechanism has a natural checkpoint (a Monitor quiet window, a wake-up tick, an empty watch interval, a missing webhook callback); the same silence that's meaningful signal for a responsive recipient on a deadline is expected for a batch-review queue that goes days between submissions.
