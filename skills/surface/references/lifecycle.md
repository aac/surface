...
**Hold-open.** The minting session stays live and drains in place — Monitor on the server's stdout, or a tight `ScheduleWakeup`/`/loop` cadence. Near-zero latency; the cost is the session idling. Correct for seconds-to-about-an-hour windows with a present recipient. Hold-open means the *turn* stays open, not merely the server: the drain only advances while the session is actively polling inside a live turn. Finalizing (ending) the turn stops the drain even though the background server keeps running and keeps accepting submissions — *server still up* is not *agent still draining*. Anything that lands after the turn ends buffers unobserved until something reopens a turn, and an environment that doesn't wake the agent on background stdout will not reopen one on its own — the classic miss is starting the server, verifying the wire, and ending the turn on the assumption the running server is still being drained. So hold-open requires Monitor blocking the turn open on the stream, or a tight within-session wake cadence that keeps re-entering the drain; if the reaction has to happen *after* this turn finalizes, that is the detached regime below, not hold-open. Note that `ScheduleWakeup` wakes *this* session — a within-session timer, not a detachment mechanism; the full transcript and accumulated context come with it on every wake. Across a multi-hour gap that is both a robustness problem (harnesses warn against resuming long-idle sessions) and a cost one (re-reading a day of transcript per wake).
...
Useful touches for this shape: bind the port *before* daemonizing, so a conflict still surfaces to the launching shell (after the double-fork, stderr is gone); a first-valid-submission-wins guard that re-derives "already decided" from the result file's existence at startup, so a restart doesn't re-record; and a self-shutdown a few seconds after recording, which lets the HTTP response flush before the process exits clean. The honesty rule (Designing affordances Rule 4) binds harder here than anywhere else: with no agent to react, the confirmation copy and any escape-hatch free-text can only be *saved to a file* — the page says "saved for the operator," never "an agent will pick this up," because none will. This is the far end of the regime axis — no held-open session, no fresh agent, just a detached process that outlives its author. Reach for it only when the environment genuinely can't run an agent at submit time, and remember that state kept in a harness-managed scratch/job directory inherits that harness's cleanup lifecycle, so write the durable result somewhere the harness won't reap.
