// surface reference server — implements the HTTP+JSON wire described in
// references/wire-example.md. Stdlib only.
//
// Usage:
//
//	go run examples/server.go --state /tmp/surface-state.json --html /tmp/surface.html [--port 5173] [--bind 127.0.0.1] [--drain-mode stdout|fs]
//
...
// handler implements the surface wire (GET /, POST /submit). The /static/<path>
...
func (h *handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case r.Method == http.MethodGet && r.URL.Path == "/":
		// no-store guards against stale-tab hazard: if a previous surface left
...
func atomicWrite(path string, data []byte) error {
	...
	tmp, err := os.CreateTemp(dir, ".surface-state-*.tmp")
	if err != nil {
		return err
	}
	tmpName := tmp.Name()
...
func (h *handler) submitMultipart(w http.ResponseWriter, r *http.Request) {
	...
	// os.TempDir(). Path shape: <TempDir>/surface-uploads/<random-hex>-<safe-name>.
	...
	savedPaths := []string{}
	uploadDir := filepath.Join(os.TempDir(), "surface-uploads")
...
func main() {
	...
	default:
		fmt.Fprintf(os.Stderr, "surface: unknown --drain-mode %q (want stdout|fs)\n", *drain)
		os.Exit(2)
	}

	addr := fmt.Sprintf("%s:%d", *bind, *port)
	fmt.Fprintf(os.Stderr, "surface: serving %s on http://%s/ (state=%s drain=%s)\n", *html, addr, *state, *drain)
	...
	if err := srv.ListenAndServe(); err != nil && !errors.Is(err, http.ErrServerClosed) {
		fmt.Fprintf(os.Stderr, "surface: server error: %v\n", err)
		os.Exit(1)
	}
}
...
func watchParentDeath(srv *http.Server, originalPPID int, tick time.Duration) {
	...
	for {
		...
		if os.Getppid() != originalPPID {
			fmt.Fprintln(os.Stderr, "surface: parent process exited; shutting down")
